...
cleanup_platform_ingress() {
    ...
    kubectl delete -f k8s-config/ingress-nexus-registry.yaml --ignore-not-found &>/dev/null || true
    ...
    if kubectl -n nexus get ingress nexus-registry-ingress &>/dev/null; then
        kubectl -n nexus wait --for=delete ingress/nexus-registry-ingress --timeout=5m &>/dev/null || true
    fi
